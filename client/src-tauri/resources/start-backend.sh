...
SERVER_CMD="${VIDEO_REPLICA_SERVER_CMD:-uv --cache-dir .uv-cache run --project server --locked uvicorn app.main:app --app-dir server --host 127.0.0.1 --port 8000}"
WORKER_CMD="${VIDEO_REPLICA_WORKER_CMD:-uv --cache-dir .uv-cache run --project server --locked python -m app.generation_worker}"

"$SERVER_CMD" > api.log 2>&1 &
SERVER_PID=$!
