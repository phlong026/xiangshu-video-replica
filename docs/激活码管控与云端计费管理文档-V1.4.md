...
> 文档日期：2026-08-19
>
> 状态：唯一架构与管理正本，可据此拆分开发任务
>
> 替代：`docs/激活码管控与云端计费架构方案.md` V1.3 及其以前版本
>
> 下游清单：`docs/短视频复刻云化改造-V1开发任务清单.md`
>
> 目标：以尽可能少的新组件，把现有单机产品改造成“瘦客户端 + 云端执行 + 独立管理端 + 激活码管控 + 按条计费”的可运营系统
...
系统上传到云端后，可以提供独立的 Web 管理端，对 Provider、套餐、激活码、设备、充值、退款、免费配额、并发参数、审计和报表进行可视化配置。

但管理端不能成为“任意修改系统内部常量”的万能配置器。配置必须分成三类：

1. **运营可配置**：套餐、激活码、设备、充值、退款、免费配额、业务并发上限。
...
| 人物身份列表当前可无条件全量读取 | `server/app/character_identity.py::list_person_identities` | 必须按当前用户过滤 |
...
| [getlago/lago](https://github.com/getlago/lago) | 参考不可变流水与幂等设计 | AGPL 且是完整计费平台；本项目只有“整数条数 + 人工充值” |
...
| 组件 | 职责 | 不承担 |
| --- | --- | --- |
| 客户桌面端 | 激活、业务操作、任务查看、余额与流水只读展示 | Provider 密钥、发码、充值、退款、调账 |
...
| 资金操作 | 充值录单、全额退款、调账、证据附件/凭证号 | admin；高风险操作二次认证 |
...
| 审计 | 登录、配置、发码、换绑、充值、退款、调账、导出 | admin/auditor 只读 |
| 报表 | 用量、充值、退款、余额、失败率、Provider 成本参考 | admin/auditor 只读 |
...
复用 `users` 表的 `admin`、`auditor` 角色，补充：
...
充值、退款、负向调账、禁用 license、换绑、Provider 付费测试、密钥变更、管理员变更均要求：
...
3. 校验状态、到期时间和已激活设备数；
4. 首次激活时创建/绑定唯一 `bound_user_id` 并发放初始权益；
...
| EXPIRED | ACTIVE | 历史读取、下载、余额与流水；禁止新建任务、上传和新 Provider 调用 |
...
累计充值、累计消费和累计退款从账本聚合，不维护易漂移的重复总计字段。
...
账本禁止 UPDATE/DELETE。除应用权限外，使用 PostgreSQL trigger 或专用数据库权限阻止修改历史流水。
...
V1 退款只支持全额信用返还，并通过 `source_transaction_id` 指向原 CHARGE；同一来源只允许一个 REFUND。部分退款和自动支付退款留到 V2。
...
| `/api/control/recharges/*` | 充值录单 |
| `/api/control/refunds/*` | 全额退款 |
...
所有列表接口必须支持服务端分页、过滤和稳定排序，避免管理端一次加载全部数据。所有写接口支持 `Idempotency-Key`。
...
| `scheduler` | 同一应用镜像 + Supercronic，执行 CLI |
