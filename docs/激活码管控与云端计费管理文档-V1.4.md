...
V1 的 REFUND 只表示“退还某一条已经 SETTLE 的视频扣费”：通过 `source_transaction_id` 指向原 SETTLE，同一 SETTLE 只允许一次全额 REFUND。它不是支付渠道的现金退款，也不能指向 CHARGE 后把充值翻倍。现金退款继续由财务线下处理；如果未来需要自动支付退款，再在 V2 单独建支付退款状态机。
